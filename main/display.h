 ...
 *   拉到 288 之后 PAR = 288/256 = 1.125，误差只剩 1.6% —— 这是本项目第一次
 *   把宽高比做对。（顺带一提：拉满 320 反而是 PAR 1.25，偏宽 9%，更差。）
 *
 * 288 = 256 * 9/8，即每 8 个源像素输出 9 个。选 9:8 而不是别的比例，是因为
 * NES 图块本来就是 8 像素宽且对齐的，复制点正好落在图块边界上，视觉最不
 * 突兀，而且内循环能 8x 展开、无分支无取模（见 nes_emu.c 的 nes_strip）。
 *
 * ---- 为什么不能再大 ----
 *
 * 卡的不是内存（见下），是 SPI 带宽。288x224x2 = 126 KB/帧，80MHz 下纯传输
 * 12.90 ms，加上每条带 122us 的驱动开销（7 条 = 0.86 ms）约 13.8 ms，
 * 已经占掉 60fps 预算 16.67 ms 的 83%。
 * 铺满 320x240 要 16.2 ms，占 97%，只能靠掉到 30fps 换 —— 而 NES 跳帧会让
 * 靠逐帧轮换来实现的「精灵闪烁」永久消失（超过 8 精灵/扫描线的那些），
 * 代价远大于消掉那 16 像素黑边。
 */
