 ...
 *
 * 跑 NES 时画布只覆盖 NES 画面区，居中放，四周黑边由 display_init() 在开机时
 * 清一次、之后再不碰。好处：每帧少推数据、静止的黑边不用重画，最重要的是
 * 省出内部 RAM 让 NES 的 vidbuf（65 KB）留得住内部 SRAM（见 nes_emu.c）。
 *
 * ---- 192 这个高度是实测顶出来的，不是拍脑袋 ----
 *
 * NES 裁掉 overscan 后是 256x224，面板有 240 行 —— 论面积明明放得下 1:1。
 * 卡住的是内部 RAM：两块帧缓冲必须都在内部 SRAM（PSRAM 走不了 DMA，
 * 原因见 display.c 里的注释），而只有一个堆区能凑出这么大的连续块：
 *
 *     268 KB  app_main 时该区最大连续空闲块（标称 292 KB，扣掉启动期已用的）
 *   -  64 KB  NES vidbuf（272x240，PPU 最热的内存，必须在内部）
 *   = 204 KB  留给两块帧缓冲 -> 每块 102 KB -> 上限约 200 行
 *
 * 取 192 行，留 12 KB 余量给 nofrendo 自己的小块分配 —— 开了 PSRAM 之后
 * 16 KB 以下的 malloc 仍然优先走内部 RAM（CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL），
 * 顶满会把它们挤到别处去。
 * 224 行（2x112 KB）实测直接分配失败，208 行（2x104 KB）也失败。
 *
 * 于是竖向抽行 224 -> 192，即每 7 行丢 1 行（见 nes_emu.c）。
 * 比换屏前那块 320x170 屏的「每 4 行丢 1 行」温和不少，画面也高了 14%。
 *
 * 想真正 1:1 铺到 224 行，只能改架构：改单缓冲 + 边转换边推的追赶式推屏，
 * 省下整整一块 100 KB，代价是每帧从 max(画图, 推屏) 变成两者首尾相接，
 * 余量从现在的一半掉到 10% 左右。要接 I2S 音频的话得先想清楚。
 *
 * 想让画布铺满整屏（比如做菜单或者点屏诊断），把这两个改成 DISP_W / DISP_H
 * —— 但那要 2x150 KB，内部 RAM 装不下，只能先改成单缓冲。
 */
#define DISP_FB_W       256
#define DISP_FB_H       192
